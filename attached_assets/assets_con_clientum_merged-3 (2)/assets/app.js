...
function initServiceCatalog(){
  ...
  function render(){
    ...
    root.innerHTML = slice.map(function(i){
      var price;
      if(i.pf === 'clientum_range' && i.pmin != null && i.pmax != null){
        price = '$ ' + i.pmin.toLocaleString('es-AR') + ' – $ ' + i.pmax.toLocaleString('es-AR') + ' ARS'
          + (i.pu ? ' <span class="price-usd">(~US$ ' + i.pu.toLocaleString('es-AR', {minimumFractionDigits:0, maximumFractionDigits:0}) + ')</span>' : '');
      } else if(i.p){
        price = '$ ' + i.p.toLocaleString('es-AR') + ' ARS'
          + (i.pu ? ' <span class="price-usd">(US$ ' + i.pu.toLocaleString('es-AR', {minimumFractionDigits:2, maximumFractionDigits:2}) + ')</span>' : '')
          + (i.pf === 'est' ? ' <span class="price-est" title="Precio estimado (mediana de categoría)">*</span>' : '');
      } else {
        price = 'Cotizar';
      }
      return '<div class="svc-card' + (i.o === 'clientum' ? ' svc-card-package' : '') + '">' +
        '<span class="cat">' + escapeHtml(i.c) + '</span>' +
        (i.tier ? '<span class="tier">' + escapeHtml(i.tier) + '</span>' : '') +
