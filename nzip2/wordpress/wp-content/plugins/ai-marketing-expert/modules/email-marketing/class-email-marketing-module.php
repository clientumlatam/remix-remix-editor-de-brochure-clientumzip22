...
class EmailMarketingModule extends Module {
	...
	public function init(): void {
		...
		// Chatbot lead capture → create/update subscriber automatically.
		add_action( 'aime_chatbot_lead_captured', array( $this, 'handle_chatbot_lead' ), 10, 1 );
	}

	/**
	 * Registers (or updates) a subscriber from a chatbot-captured lead so it
	 * flows into list/tag segmentation and automation funnels automatically.
	 *
	 * @param array $lead_data Lead payload from LeadCaptureService::process_lead().
	 */
	public function handle_chatbot_lead( array $lead_data ): void {
		$controller    = new Controllers\SubscriberController();
		$subscriber_id = $controller->create_from_lead( $lead_data );

		if ( $subscriber_id ) {
			aime_log(
				sprintf( 'Chatbot lead registered as subscriber #%d (%s)', $subscriber_id, $lead_data['email'] ?? '' ),
				'info',
				'email-marketing'
			);
		}
