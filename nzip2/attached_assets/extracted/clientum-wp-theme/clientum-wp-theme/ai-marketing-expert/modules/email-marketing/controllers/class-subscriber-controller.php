...
class SubscriberController {
	...
	/**
	 * Create or update a subscriber from an external lead source (e.g. the
	 * chatbot module). Reuses the same list/tag pivot + activity-log helpers
	 * as the other creation paths so leads behave like any other subscriber.
	 *
	 ...
	 *     @type array  $metadata
	 * }
	 * @return int Subscriber ID, or 0 if the email was invalid.
	 */
	public function create_from_lead( array $lead_data ): int {
		global $wpdb;
		$subscribers_table = $wpdb->prefix . 'aime_subscribers';
		...
		$first_name = $this->sanitize_limited_text( $lead_data['first_name'] ?? '', 120 );
		$source     = sanitize_text_field( $lead_data['source'] ?? 'lead' );
		$list_id    = absint( $lead_data['list_id'] ?? 0 );
		$tag_ids    = $this->resolve_tag_names( (array) ( $lead_data['tags'] ?? array() ) );

		$existing = $wpdb->get_row( $wpdb->prepare( 'SELECT id, first_name, status FROM %i WHERE email = %s', $subscribers_table, $email ) );

		if ( $existing ) {
			$id = (int) $existing->id;

			if ( $first_name && empty( $existing->first_name ) ) {
				$wpdb->update( $subscribers_table, array( 'first_name' => $first_name ), array( 'id' => $id ) );
			}

			if ( $list_id ) {
				$this->attach_pivot( $id, array( $list_id ), 'list' );
			}
			if ( $tag_ids ) {
				$this->attach_pivot( $id, $tag_ids, 'tag' );
			}
			if ( ! empty( $lead_data['metadata'] ) ) {
				$this->save_meta( $id, array_map( 'sanitize_text_field', array_filter( (array) $lead_data['metadata'] ) ) );
			}

			$this->log_activity( $id, 'lead_matched', sprintf( 'Matched existing subscriber from %s lead', $source ) );
		...
		$this->log_activity( $id, 'created', sprintf( 'Subscribed via %s lead', $source ) );
