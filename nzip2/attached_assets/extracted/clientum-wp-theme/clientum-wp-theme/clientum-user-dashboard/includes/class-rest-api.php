...
/**
 * Cifrado simétrico simple para secretos guardados en user_meta (p.ej. API keys
 * de WhatsApp). Usa las claves de seguridad de WordPress (wp-config.php) como
 * material de cifrado, así el valor nunca queda en texto plano en la base de datos.
 */
class Clntm_Crypto {
    ...
    public static function encrypt( $plain ) {
        ...
        $iv  = random_bytes( 16 );
        $enc = openssl_encrypt( (string) $plain, 'aes-256-cbc', self::key(), OPENSSL_RAW_DATA, $iv );
        if ( $enc === false ) return '';
...
class Clntm_Rest_Api {
    ...
    public function whatsapp_connect( WP_REST_Request $req ) {
        ...
        update_user_meta( $uid, 'clntm_whatsapp', [
            'phone'      => $phone,
            'api_key'    => $apiKey ? Clntm_Crypto::encrypt( $apiKey ) : '',
            'connected'  => true,
