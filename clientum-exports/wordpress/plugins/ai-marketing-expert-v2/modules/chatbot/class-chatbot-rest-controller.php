...
use WPSpace\AiMarketingExpert\Modules\Chatbot\Controllers\WhatsAppController;
...
class ChatbotRestController {
	...
	private function register_whatsapp_routes(): void {
		...
		register_rest_route( $this->ns, '/chatbot/whatsapp/status', array(
			'methods'             => 'GET',
			'callback'            => array( $c, 'status' ),
			'permission_callback' => array( $this, 'admin_permission' ),
		) );

		// GET /chatbot/whatsapp/qr
		register_rest_route( $this->ns, '/chatbot/whatsapp/qr', array(
			'methods'             => 'GET',
			'callback'            => array( $c, 'qr' ),
			'permission_callback' => array( $this, 'admin_permission' ),
		) );

		// POST /chatbot/whatsapp/connect
		register_rest_route( $this->ns, '/chatbot/whatsapp/connect', array(
			'methods'             => 'POST',
			'callback'            => array( $c, 'connect' ),
			'permission_callback' => array( $this, 'admin_permission' ),
		) );

		// POST /chatbot/whatsapp/logout
		register_rest_route( $this->ns, '/chatbot/whatsapp/logout', array(
			'methods'             => 'POST',
			'callback'            => array( $c, 'logout' ),
			'permission_callback' => array( $this, 'admin_permission' ),
		) );

		// GET /chatbot/whatsapp/settings
		register_rest_route( $this->ns, '/chatbot/whatsapp/settings', array(
			'methods'             => 'GET',
			'callback'            => array( $c, 'get_settings' ),
			'permission_callback' => array( $this, 'admin_permission' ),
		) );

		// POST /chatbot/whatsapp/settings
		register_rest_route( $this->ns, '/chatbot/whatsapp/settings', array(
			'methods'             => 'POST',
			'callback'            => array( $c, 'save_settings' ),
			'permission_callback' => array( $this, 'admin_permission' ),
		...
		// POST /chatbot/whatsapp/incoming — webhook called by the Node bridge, secured by shared secret (not a WP user).
		register_rest_route( $this->ns, '/chatbot/whatsapp/incoming', array(
			'methods'             => 'POST',
			'callback'            => array( $c, 'incoming' ),
