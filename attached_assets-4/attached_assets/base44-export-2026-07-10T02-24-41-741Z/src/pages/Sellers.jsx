...
const SellerCard = ({ seller, onEdit }) => {
...
const SellerForm = ({ seller, onSave, onCancel, isLoading }) => {
  const [form, setForm] = useState(seller || { name: '', phone: '', email: '', specialty: 'general', branch: '', active: true });
...
export default function Sellers() {
  ...
  const saveMutation = useMutation({
    mutationFn: (data) => editing?.id
      ? db.entities.Seller.update(editing.id, data)
      : db.entities.Seller.create(data),
    onSuccess: () => { qc.invalidateQueries(['sellers']); setEditing(null); setShowForm(false); },
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        <Button
          variant={filterSpec === 'all' ? 'default' : 'outline'}
          size="sm"
        ...
        {specialties.map(s => {
          ...
          return (
            <Button
              key={s.value}
              variant={filterSpec === s.value ? 'default' : 'outline'}
              size="sm"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {isLoading ? (
          Array(6).fill(0).map((_, i) => <Card key={i} className="h-40 animate-pulse bg-muted border-0" />)
        ) : (
          filtered.map(s => (
            <SellerCard
              key={s.id}
              seller={s}
              onEdit={(seller) => { setEditing(seller); setShowForm(true); }}
            />
          ))
        )}
      </div>
