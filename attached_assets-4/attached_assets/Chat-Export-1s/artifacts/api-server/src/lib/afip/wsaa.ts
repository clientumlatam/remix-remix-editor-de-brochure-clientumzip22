...
export async function getToken(tenantId: number): Promise<{ token: string; sign: string; cuit: string; puntoVenta: number }> {
  const [config] = await db.select().from(afipConfigsTable).where(eq(afipConfigsTable.tenantId, tenantId)).limit(1);
  if (!config) throw new Error("AFIP no configurado para este tenant");
  ...
  await db.update(afipConfigsTable)
    ...
    .where(eq(afipConfigsTable.tenantId, tenantId));

  return { token, sign, cuit: config.cuit, puntoVenta: config.puntoVenta };
}

export async function refreshToken(tenantId: number): Promise<void> {
  const [config] = await db.select().from(afipConfigsTable).where(eq(afipConfigsTable.tenantId, tenantId)).limit(1);
